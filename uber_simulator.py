...
class Theme:
    ...
    MAP_BG = "#1A2332"
    MAP_STREET = "#2D3A4A"
    MAP_STREET_LINE = "#4A5A6A"
    MAP_BUILDING_1 = "#2C3E50"
    MAP_BUILDING_2 = "#34495E"
    MAP_BUILDING_3 = "#3D566E"
    MAP_PARK = "#2D5A3D"
    MAP_PARK_TREE = "#3A7A50"
    MAP_WATER = "#1A3A5C"
    PICKUP_COLOR = "#00C853"
    DEST_COLOR = "#E94560"
    CAR_COLOR = "#276EF1"
...
class UberRideSimulator:
    ...
    def _setup_fonts(self):
        self.font_logo = tkfont.Font(family=Theme.FONT_FAMILY, size=42, weight="bold")
    ...
    def _build_ui(self):
        ...
        tk.Label(self.top_bar, text="UBER", font=self.font_logo,
                 fg=Theme.TEXT, bg=Theme.SURFACE).pack(side="left", padx=24, pady=4)
    ...
    def _make_btn(self, parent, text, command, bg=Theme.PRIMARY, fg=Theme.TEXT,
        ...
        btn = tk.Button(parent, text=text, command=command,
                        bg=bg, fg=fg, font=font or self.font_btn,
                        relief="flat", bd=0,
        ...
        # Hover effects
        def on_enter(e):
            if e.widget["bg"] != Theme.SECONDARY:
                e.widget["bg"] = lerp_color(bg, "#FFFFFF", 0.12)
        def on_leave(e):
            e.widget["bg"] = bg
    ...
    def show_welcome(self):
        ...
        for ch in "UBER":
            lbl = tk.Label(logo_box, text=ch, font=self.font_logo,
    ...
    def show_driver_arriving(self):
        ...
        # Create the trip map
        self.trip_map = RideMap(map_container, width=900, height=500, bg=Theme.SURFACE)
    ...
    def _animate_car_to_pickup(self):
        ...
        total_steps = len(path) * 10  # 10 frames per segment
    ...
    def _start_trip_phase(self):
        ...
        # Hide driver info bar, show trip progress
        for w in self.content.winfo_children():
            if isinstance(w, tk.Frame) and w.winfo_children():
                for child in w.winfo_children():
                    if hasattr(child, 'cget') and child.winfo_class() == "Frame" and \
                       child.winfo_height() == 60:
                        child.destroy()
                        break

        # Trip progress bar at top
        self._trip_progress = tk.Frame(self.content, bg=Theme.SURFACE, height=50)
        self._trip_progress.pack(fill="x", before=self.content.winfo_children()[0] if self.content.winfo_children() else None)
...
class RideMap(tk.Frame):
    """An interactive city map with buildings, roads, and car animation."""
    ...
    def __init__(self, parent, width=700, height=500, **kw):
        ...
        self.canvas = tk.Canvas(self, width=width, height=height,
                                 bg=Theme.MAP_BG, highlightthickness=0)
        self.canvas.pack()
    ...
    def draw_city(self):
        ...
        if self.pickup_pt:
            self._draw_pin(self.pickup_pt[0], self.pickup_pt[1], Theme.PICKUP_COLOR, "PICKUP")
        if self.dest_pt:
            self._draw_pin(self.dest_pt[0], self.dest_pt[1], Theme.DEST_COLOR, "DROP")
    ...
    def _draw_roads(self):
        ...
        for x1, y1, x2, y2 in MAP_ROADS_H:
            road_w = 24
            self.canvas.create_rectangle(x1, y1 - road_w // 2, x2, y2 + road_w // 2,
                                          fill=Theme.MAP_STREET, outline="")

        # Vertical roads
        for x1, y1, x2, y2 in MAP_ROADS_V:
            road_w = 24
            self.canvas.create_rectangle(x1 - road_w // 2, y1, x2 + road_w // 2, y2,
                                          fill=Theme.MAP_STREET, outline="")

    def _draw_road_lines(self):
        """Draw dashed center lines on roads."""
        # Horizontal
        for x1, y1, x2, y2 in MAP_ROADS_H:
            cy = (y1 + y2) / 2
            for x in range(int(x1), int(x2), 20):
                self.canvas.create_line(x, cy, x + 10, cy,
                                        fill=Theme.MAP_STREET_LINE, width=1)

        # Vertical
        for x1, y1, x2, y2 in MAP_ROADS_V:
            cx = (x1 + x2) / 2
            for y in range(int(y1), int(y2), 20):
                self.canvas.create_line(cx, y, cx, y + 10,
                                        fill=Theme.MAP_STREET_LINE, width=1)

    def _draw_blocks(self):
        """Draw city blocks (buildings, parks, water)."""
        for bx, by, bw, bh, color, label in CITY_BLOCKS:
            # Building
            self.canvas.create_rectangle(bx, by, bx + bw, by + bh,
                                          fill=color, outline=lerp_color(color, "#000", 0.2),
                                          width=1)

            # If park, add some trees
            if color == Theme.MAP_PARK:
                for _ in range(3):
                    tx = bx + random.randint(10, bw - 10)
                    ty = by + random.randint(10, bh - 10)
            ...
            # If water, add waves
                ...
                for wx in range(bx + 10, bx + bw - 10, 20):
                    wy = by + bh // 2
                    self.canvas.create_line(wx, wy, wx + 10, wy - 4, wx + 20, wy,
                                            fill=Theme.MAP_WATER, width=1,
                                            smooth=True)
    ...
    def _draw_labels(self):
        ...
        for bx, by, bw, bh, color, label in CITY_BLOCKS:
            if label:
                ...
                text_bg = lerp_color(color, "#000", 0.5)
                self.canvas.create_text(cx_text, cy_text, text=label,
                                         fill=Theme.TEXT_SEC, font=("Helvetica", 8, "bold"))

    def _draw_pin(self, x, y, color, label):
        """Draw a location pin/marker."""
        # Outer glow
        self.canvas.create_oval(x - 14, y - 14, x + 14, y + 14,
                                fill=lerp_color(color, "#FFF", 0.3), outline="")
        # Pin circle
    ...
    def move_car(self, x, y):
        """Move the car to a position on the map."""
        if self.car_id:
        ...
        # Windshield (small lighter rectangle)
        self.canvas.create_rectangle(
            x - 3, y - size // 2 + 2, x + 3, y + size // 2 - 2,
            fill=lerp_color(self.car_color, "#FFF", 0.4), outline="")

        # Wheels
        wheel_color = "#333"
        self.canvas.create_oval(x - size + 2, y - size // 2 - 2,
                                 x - size + 6, y - size // 2 + 2,
                                 fill=wheel_color, outline="")
        self.canvas.create_oval(x + size - 6, y - size // 2 - 2,
                                 x + size - 2, y - size // 2 + 2,
                                 fill=wheel_color, outline="")
        self.canvas.create_oval(x - size + 2, y + size // 2 - 2,
                                 x - size + 6, y + size // 2 + 2,
