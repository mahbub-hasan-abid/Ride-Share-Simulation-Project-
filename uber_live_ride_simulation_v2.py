...
class UberLiveSimulation(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("Uber Live Ride Simulation")
    ...
    def _build_sidebar(self):
        ...
        top = tk.Frame(panel, bg=self.SURFACE)
        top.pack(fill="x", padx=20, pady=(20, 12))

        self._label(top, "Where to?", size=24, bold=True).pack(anchor="w")
        self._label(
            top,
            "Select a destination or tap directly on the map.",
            size=9,
            color=self.MUTED,
        ).pack(anchor="w", pady=(3, 0))

        form = tk.Frame(panel, bg=self.SURFACE)
        ...
        self.passenger_entry.pack(fill="x", ipady=9, pady=(0, 11))
        ...
        self.pickup_combo.pack(fill="x", pady=(0, 11))
        ...
        self.destination_combo.pack(fill="x", pady=(0, 11))
        self.destination_combo.bind("<<ComboboxSelected>>", self._on_destination_selected)

        self.drop_hint = tk.Label(
            form,
            text="Tip: click anywhere on the map to drop the destination pin.",
            bg="#EFF6FF",
            fg=self.BLUE,
            anchor="w",
            justify="left",
        ...
        self.drop_hint.pack(fill="x", pady=(0, 12))
        ...
        self.payment_combo.pack(fill="x", pady=(0, 13))

        line = tk.Frame(panel, bg=self.BORDER, height=1)
        line.pack(fill="x", padx=20, pady=(2, 12))

        vehicle_header = tk.Frame(panel, bg=self.SURFACE)
        vehicle_header.pack(fill="x", padx=20)
        self._label(vehicle_header, "Choose a ride", size=13, bold=True).pack(side="left")
        self.live_label = self._label(vehicle_header, "Live prices", size=9, color=self.GREEN, bold=True)
        self.live_label.pack(side="right")

        self.vehicle_list = tk.Frame(panel, bg=self.SURFACE)
        self.vehicle_list.pack(fill="x", padx=16, pady=(8, 8))

        for vehicle_name in self.VEHICLE_QUOTES:
            self._create_vehicle_card(vehicle_name)

        self.fare_summary = tk.Frame(panel, bg="#F6F7F8", padx=14, pady=11)
        self.fare_summary.pack(fill="x", padx=20, pady=(2, 10))
        ...
        self.request_button.pack(fill="x", padx=20, pady=(0, 8))
        ...
        self.reset_button.pack(fill="x", padx=20, pady=(0, 16))
    ...
    def _create_vehicle_card(self, vehicle_name):
        ...
        icon_canvas = tk.Canvas(card, width=52, height=48, bg=self.SURFACE, highlightthickness=0)
        icon_canvas.pack(side="left", padx=(8, 4), pady=5)
        self._draw_car_icon(icon_canvas, 26, 24, quote.color, quote.code, scale=1.0)

        middle = tk.Frame(card, bg=self.SURFACE)
        middle.pack(side="left", fill="both", expand=True, pady=7)
        ...
        price_label.pack(side="right", padx=12)

        elements = [card, icon_canvas, middle, name_label, eta_label, price_label]
        for widget in elements:
            widget.bind("<Button-1>", lambda _e, v=vehicle_name: self.select_vehicle(v))
    ...
    def select_vehicle(self, vehicle_name):
        ...
        old = self.selected_vehicle
        self.selected_vehicle = vehicle_name
        self._style_vehicle_card(old, selected=False)
        self._style_vehicle_card(vehicle_name, selected=True)
        self.request_button.config(text=f"CONFIRM {vehicle_name.upper()}")
        self._refresh_quotes()
    ...
    def _style_vehicle_card(self, vehicle_name, selected):
        ...
        card_data["frame"].config(bg=background, highlightbackground=border, highlightthickness=2 if selected else 1)
    ...
    def _refresh_quotes(self):
        ...
        for vehicle_name, quote in self.VEHICLE_QUOTES.items():
            price = self._quote_for_vehicle(vehicle_name)
            eta = max(1, quote.eta_min + random.choice([-1, 0, 0, 1]))
        ...
        if self.destination_point is None:
            ...
            self.summary_route.config(
                text=f"{self.pickup_name.get()} → {self.destination_label}  ·  {distance:.1f} km"
            )
            self.summary_price.config(text=f"Estimated fare: ৳{self.current_fare:,.0f}")
            selected_eta = self.VEHICLE_QUOTES[self.selected_vehicle].eta_min
            self.map_info_text.config(text=f"{self.selected_vehicle} available in about {selected_eta} min")

        # Update price bubbles on map.
        for vehicle in self.live_vehicles.values():
            price = self._quote_for_vehicle(vehicle["type"])
            vehicle["price"] = price
            self.canvas.itemconfig(vehicle["price_item"], text=f"৳{price:,.0f}")
        ...
        if not self.ride_active:
            self.quote_job = self.after(2800, self._refresh_quotes)
    ...
    def reset_simulation(self):
        ...
        self.request_button.config(
            text=f"CONFIRM {self.selected_vehicle.upper()}",
            state="normal",
        )
